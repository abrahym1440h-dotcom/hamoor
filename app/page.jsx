...
import {
  ...
  ChevronRight, Share2, Trash2, Archive, FileText, Eye
} from "lucide-react";
...
function useScreenSize() {
  ...
  useEffect(() => {
    function handleResize() {
      ...
      setSize({ width: w, isMobile: w < 768, isTablet: w >= 768 && w < 1280, isDesktop: w >= 1280 });
    }
    handleResize();
            ...
            <Card style={{boxShadow:SH.lift,marginTop:sp[5]}}>
              ...
              <div style={{padding:`${sp[4]}px ${sp[5]}px ${sp[5]}px`}}>
                ...
                <div style={{marginBottom:sp[4]}}>
                  ...
                  <textarea value={details} onChange={e=>setDetails(e.target.value)} placeholder="مثال: كوفي بأجواء يابانية" rows={3} style={{...iStyle, resize:"none", fontFamily:"inherit", lineHeight:1.5}}/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:screen.isMobile?"1fr":"1fr 1fr",gap:sp[3]}}>
                  ...
                  <FormField label="الحي" icon={<MapPin size={14} color={$.L4}/>}>
                    <input value={neighborhood} onChange={e=>setNeighborhood(e.target.value)} placeholder="اختياري" style={iStyle}/>
                  </FormField>
                </div>
              ...
              {savedCount > 0 && (
                <Card onClick={onViewSaved} style={{cursor:"pointer",padding:`${sp[4]}px ${sp[5]}px`,display:"flex",alignItems:"center",gap:sp[3]}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:$.L3}}>{savedCount} تحليلات محفوظة</div>
                  </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:screen.isDesktop?"1fr 1fr":"1fr",gap:sp[4]}}>
            {tab===0 && (<>
              ...
              <div style={{gridColumn:screen.isDesktop?"span 2":"auto"}}>
                {result.recommendations?.length>0 && <Section title="التوصيات" Icon={Lightbulb} color={$.purple}>{result.recommendations.map((s,i)=><div key={i} style={{display:"flex",alignItems:"flex-start",gap:sp[3],marginBottom:sp[3],background:`${$.purple}06`,padding:`${sp[3]}px ${sp[4]}px`,borderRadius:10}}><div style={{width:22,height:22,borderRadius:"50%",background:$.purple,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{i+1}</div><span style={{fontSize:14,color:$.L2,lineHeight:1.6}}>{s}</span></div>)}</Section>}
              </div>
            </>)}
            ...
            {tab===1 && (<>
              ...
              {m.competitors?.length>0 && <Section title="المنافسون" Icon={Briefcase} color={$.orange}>{m.competitors.map((c,i)=><div key={i} style={{padding:`${sp[3]}px 0`,borderBottom:i<m.competitors.length-1?`0.5px solid ${$.sepL}`:"none"}}><div style={{display:"flex",alignItems:"center",gap:sp[2],marginBottom:4}}><Star size={14} color={$.orange}/><span style={{fontSize:14,fontWeight:700,color:$.L1}}>{c.name}</span></div><p style={{fontSize:13,color:$.L3,lineHeight:1.5,paddingRight:sp[5]}}>{c.strength}</p></div>)}</Section>}
            </>)}
            ...
            {tab===2 && (<>
              ...
              <Section title="توقع الإيرادات" Icon={TrendingUp} color={$.green}>
                ...
                <MoneyRow label="السنة الثانية" value={rp.year_2_monthly}/>
                <MoneyRow label="السنة الثالثة" value={rp.year_3_monthly} valueColor={$.green} bold/>
              </Section>
                  ...
                  {(result.risk_analysis||[]).map((r,i)=>{
                    ...
                    return (
                      <div key={i} style={{background:$.F5,borderRadius:14,padding:`${sp[4]}px`,marginBottom:sp[3]}}>
                        ...
                        <div style={{display:"flex",gap:sp[2],marginBottom:sp[3],flexWrap:"wrap"}}><Chip text={"احتمالية: "+r.probability} color={probColor} bg={`${probColor}15`}/></div>
      ...
      <div style={containerStyle}>
                ...
                <div onClick={()=>onViewAnalysis(a)} style={{padding:`${sp[4]}px ${sp[5]}px`,cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:sp[4],marginBottom:sp[3]}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:12,color:$.L3,display:"flex",alignItems:"center",gap:sp[2]}}>
                        <MapPin size={11}/><span>{a.city}</span>
                      </div>
                    </div>
                  </div>
        ...
        <Sheet open={!!confirmDelete} onClose={()=>setConfirmDelete(null)}>
          <div style={{padding:`${sp[5]}px ${sp[5]}px ${sp[8]}px`,textAlign:"center"}}>
            ...
            <p style={{fontSize:14,color:$.L3,lineHeight:1.6,marginBottom:sp[6]}}>سيتم حذف هذا التحليل نهائياً</p>
...
const SECTORS_DATA = [
  {id:1, category:"food", name:"مقاهي ومشروبات", Icon:Coffee, color:$.orange, score:68, growth:"+12%", failure_rate:"60%", investment:"150,000 - 400,000", payback:"18-24 شهر", margin:"15-25%", competition:"عالية جداً", audience:"شباب 18-35، طلاب، موظفون", top_cities:["الرياض","جدة","الخبر","الدمام"], success_tips:["موقع استراتيجي","تميّز في القهوة","تصميم جذاب للسوشيال","خدمة توصيل سريعة","برامج ولاء"], failure_reasons:["إشباع السوق","موقع ضعيف","ضعف التمييز","تكاليف عالية"], competitors:["ستاربكس","عربيكا","دنكن","مذاق"], sub_ideas:["كوفي كوري","عربة قهوة متنقلة","كوفي تراثي"], last_updated:"يناير 2026"},
  {id:2, category:"food", name:"مطاعم وأكل", Icon:Utensils, color:$.red, score:65, growth:"+8%", failure_rate:"70%", investment:"200,000 - 800,000", payback:"24-36 شهر", margin:"10-18%", competition:"عالية", audience:"عائلات، موظفون، شباب", top_cities:["الرياض","جدة","الدمام","الخبر"], success_tips:["تخصص واضح","اتساق الجودة","توصيل قوي","تسعير منافس"], failure_reasons:["قائمة كبيرة","ضعف الإدارة المالية","منافسة شرسة","موسمية صعبة"], competitors:["البيك","كودو","هرفي","ماكدونالدز"], sub_ideas:["مطعم كبسة","مطعم آسيوي","فطور راقي"], last_updated:"يناير 2026"},
  {id:3, category:"food", name:"حلويات ومخبوزات", Icon:Cake, color:$.pink, score:72, growth:"+18%", failure_rate:"45%", investment:"120,000 - 500,000", payback:"12-18 شهر", margin:"25-40%", competition:"متوسطة", audience:"نساء، عائلات، مناسبات", top_cities:["الرياض","جدة","القصيم"], success_tips:["تصوير احترافي","تغليف فاخر","توصيل سريع","تخصص في نوع"], failure_reasons:["تقليد المنافسين","ضعف التغليف","عدم اتساق الجودة"], competitors:["صابا","ميلانو","لافيت"], sub_ideas:["حلويات صحية","كنافة فاخرة","كيكات مناسبات"], last_updated:"يناير 2026"},
  {id:4, category:"food", name:"وجبات سريعة", Icon:Pizza, color:$.yellow, score:64, growth:"+10%", failure_rate:"55%", investment:"100,000 - 350,000", payback:"15-24 شهر", margin:"20-30%", competition:"عالية", audience:"شباب، طلاب، عمال", top_cities:["الرياض","جدة","الدمام"], success_tips:["سرعة التحضير","تسعير منافس","موقع قريب من الجامعات","توصيل قوي"], failure_reasons:["منافسة شرسة","ضعف الجودة","ارتفاع التكاليف"], competitors:["ماكدونالدز","برجر كنق","البيك"], sub_ideas:["برجر سعودي","شاورما فاخرة","ساندوتشات صحية"], last_updated:"يناير 2026"},
  {id:5, category:"retail", name:"تجزئة عامة", Icon:ShoppingBag, color:$.purple, score:55, growth:"+5%", failure_rate:"55%", investment:"100,000 - 500,000", payback:"24-36 شهر", margin:"15-30%", competition:"عالية جداً", audience:"عام", top_cities:["الرياض","جدة","الدمام"], success_tips:["تخصص واضح","موقع جيد","إدارة مخزون","حضور أونلاين"], failure_reasons:["منافسة من التجارة الإلكترونية","بضاعة راكدة","موقع ضعيف"], competitors:["نون","أمازون","ساكو","جرير"], sub_ideas:["متجر أطفال","متجر حيوانات","متجر هدايا"], last_updated:"يناير 2026"},
  {id:6, category:"retail", name:"أزياء وعبايات", Icon:Shirt, color:$.indigo, score:75, growth:"+15%", failure_rate:"40%", investment:"80,000 - 300,000", payback:"12-18 شهر", margin:"25-40%", competition:"متوسطة", audience:"نساء 20-60، مناسبات", top_cities:["الرياض","جدة","الخبر"], success_tips:["تصاميم حصرية","خياطة عالية الجودة","إنستقرام احترافي","خدمة VIP"], failure_reasons:["تشابه التصاميم","تسعير ضعيف","ضعف التسويق"], competitors:["مزون","نهى","حلا الترك"], sub_ideas:["عبايات شبابية","فساتين سهرة","عبايات صلاة فاخرة"], last_updated:"يناير 2026"},
  {id:7, category:"retail", name:"إلكترونيات", Icon:Smartphone, color:$.teal, score:60, growth:"+7%", failure_rate:"50%", investment:"200,000 - 1,000,000", payback:"30-48 شهر", margin:"8-18%", competition:"عالية جداً", audience:"شباب، موظفون", top_cities:["الرياض","جدة","الدمام"], success_tips:["أسعار منافسة","ضمان موثوق","تشكيلة متنوعة","صيانة في المحل"], failure_reasons:["هامش ربح ضعيف","منافسة قاتلة","تزييف المنتجات"], competitors:["إكسترا","جرير","نون"], sub_ideas:["إكسسوارات الجوالات","صيانة متخصصة"], last_updated:"يناير 2026"},
  {id:8, category:"services", name:"صالونات وتجميل", Icon:Sparkle, color:$.pink, score:70, growth:"+14%", failure_rate:"50%", investment:"100,000 - 400,000", payback:"18-24 شهر", margin:"25-40%", competition:"متوسطة", audience:"نساء 18-55، مناسبات", top_cities:["الرياض","جدة","الخبر"], success_tips:["مصففين موهوبين","تجربة فاخرة","نظام حجز","تخصص"], failure_reasons:["دوران الموظفين","عدم النظافة","تسعير غير واضح"], competitors:["روزا","إكسير","توني آند جاي"], sub_ideas:["صالون رجالي","صالون أعراس","عيادة تجميلية"], last_updated:"يناير 2026"},
  {id:9, category:"services", name:"خياطة وتفصيل", Icon:Scissors, color:$.purple, score:62, growth:"+6%", failure_rate:"30%", investment:"40,000 - 150,000", payback:"12-18 شهر", margin:"30-50%", competition:"متوسطة", audience:"رجال، نساء، مناسبات", top_cities:["الرياض","القصيم","المدينة المنورة"], success_tips:["خياطين ماهرين","الالتزام بالمواعيد","تخصص","أقمشة فاخرة"], failure_reasons:["تأخر التسليم","ضعف الجودة","نقص الخياطين"], competitors:["محلات محلية","الطلال","الفيصلية"], sub_ideas:["فساتين سهرة","بشوت ملوكية","عبايات تصاميم خاصة"], last_updated:"يناير 2026"},
  {id:10, category:"professional", name:"تعليم وتدريب", Icon:GraduationCap, color:$.blue, score:82, growth:"+22%", failure_rate:"35%", investment:"80,000 - 350,000", payback:"12-20 شهر", margin:"35-55%", competition:"متوسطة", audience:"طلاب، موظفون", top_cities:["الرياض","جدة","الدمام"], success_tips:["مدربين ذوي خبرة","محتوى مميز","شهادات معتمدة","تسويق رقمي"], failure_reasons:["ضعف الجودة","تسعير مرتفع","عدم التخصص"], competitors:["دروب","رواق","تمكين"], sub_ideas:["برمجة للأطفال","تطوير الذات","لغات متخصصة"], last_updated:"يناير 2026"},
  {id:11, category:"professional", name:"لياقة ورياضة", Icon:Dumbbell, color:$.green, score:78, growth:"+20%", failure_rate:"40%", investment:"150,000 - 600,000", payback:"18-30 شهر", margin:"30-45%", competition:"متوسطة", audience:"شباب 18-45", top_cities:["الرياض","جدة","الخبر"], success_tips:["أجهزة حديثة","مدربين معتمدين","تنوع البرامج","نظافة"], failure_reasons:["أجهزة قديمة","اشتراكات مرتفعة","ضعف خدمة العملاء"], competitors:["فتنس تايم","بادي ماستر","نقاء"], sub_ideas:["نادي نسائي","مركز كروسفت","ستوديو يوغا"], last_updated:"يناير 2026"},
  {id:12, category:"professional", name:"خدمات تقنية", Icon:Wifi, color:$.indigo, score:85, growth:"+25%", failure_rate:"30%", investment:"50,000 - 300,000", payback:"12-18 شهر", margin:"40-60%", competition:"منخفضة", audience:"شركات، رواد أعمال", top_cities:["الرياض","جدة","الدمام"], success_tips:["تخصص","محفظة أعمال قوية","أسعار باقات واضحة","دعم فني"], failure_reasons:["عدم التخصص","ضعف التسعير","صعوبة إيجاد عملاء"], competitors:["شركات محلية","موضوع","حسوب"], sub_ideas:["تسويق رقمي","تصميم تطبيقات","إدارة سوشيال"], last_updated:"يناير 2026"}
];
